...
class Simulation:
    ...
    def validateSegments(self, session):
        ...
        #New segments are obtained by applied the longest common subsequence algorithm
    ...
    def wordCorrection(self, session):
        ...
        for n in range(1, self.reference_size):
            if self.reference_in_segment[n] == None:
                ...
                #(The target in the hypothesis that follows the segment in which
                #the previous reference--n - 1--was located.)
                self.reference_in_segment[n] = self.reference_in_segment[n - 1] + 1
                #Update reference_in_segment (a new segment is going to ve added after
                #the segment in which the previous reference was located).
        ...
        if hyp[-1] != self.reference[-1]:#If hypothesis and reference are different:
